# Looking at the MetaGeneSets from Bob's pipeline and subsetting after everything has been run - Week 2 vs broth!!
# E. Lamont
# 6/19/25

# Following from MetaGeneSets_Analysis.R, but just looking at iModulons and their categories

source("Import_data.R") # To get MetaGeneSets_W2vsBroth
...
MetaGeneSets_W2vsBroth_UP_iModulons <- MetaGeneSets_W2vsBroth_UP %>% 
...
MetaGeneSets_W2vsBroth_DOWN_iModulons <- MetaGeneSets_W2vsBroth_DOWN %>% 
  filter(str_detect(PathName, "iModulons")) %>% 
  filter(!str_detect(PathName, "ISB.Corems")) %>%
  # filter(LOG2FOLD <= 0) %>%
  mutate(PathName = PathName %>%
...
# Combine the two 
MetaGeneSets_W2vsBroth_ALL_iModulons <- rbind(MetaGeneSets_W2vsBroth_UP_iModulons %>% filter(LOG2FOLD >= 0), MetaGeneSets_W2vsBroth_DOWN_iModulons %>% filter(LOG2FOLD <= 0))
...
# Filter
MetaGeneSets_W2vsBroth_ALL_iModulons_CentralCarbon <- MetaGeneSets_W2vsBroth_ALL_iModulons %>% filter(str_detect(PathName, pattern))

ColumnPlot <- MetaGeneSets_W2vsBroth_ALL_iModulons_CentralCarbon %>% 
...
# Filter
MetaGeneSets_W2vsBroth_ALL_iModulons_AminoAcid <- MetaGeneSets_W2vsBroth_ALL_iModulons %>% filter(str_detect(PathName, pattern))

ColumnPlot <- MetaGeneSets_W2vsBroth_ALL_iModulons_AminoAcid %>% 
  mutate(PathName = str_wrap(PathName, width = 60)) %>%  # Adjust width as needed
  arrange(desc(LOG2FOLD)) %>%
  ggplot(aes(reorder(PathName, LOG2FOLD), LOG2FOLD)) + 
  geom_col(aes(fill = AVG_PVALUE < 0.05)) + 
  ...
  labs(x="Amino Acid Biosynthesis iModulon", y="LOG2FOLD",
       title="Amino Acid Biosynthesis iModulons in W2 Sputum vs Log broth \nusing UP and DOWN combined",
       subtitle = "The UP with LOG2FOLD > 0 and the DOWN with LOG2FOLD < 0") + 
  my_plot_themes + theme(axis.text.y = element_text(size=12))
ColumnPlot
ggsave(ColumnPlot,
       file = "iModulons_W2.MTb.MetaGeneSets.ALL_AminoAcid.pdf",
       path = "GSEA_Figures/MetaGeneSets/iModulons/W2vsBroth",
       width = 10, height = 6, units = "in")
...
# Filter
MetaGeneSets_W2vsBroth_ALL_iModulons_NucleiAcid <- MetaGeneSets_W2vsBroth_ALL_iModulons %>% filter(str_detect(PathName, pattern))

ColumnPlot <- MetaGeneSets_W2vsBroth_ALL_iModulons_NucleiAcid %>% 
  mutate(PathName = str_wrap(PathName, width = 60)) %>%  # Adjust width as needed
  arrange(desc(LOG2FOLD)) %>%
  ggplot(aes(reorder(PathName, LOG2FOLD), LOG2FOLD)) + 
  geom_col(aes(fill = AVG_PVALUE < 0.05)) + 
  ...
  labs(x="Nucleic Acid iModulon", y="LOG2FOLD",
       title="Nucleic Acid iModulons in W2 Sputum vs Log broth \nusing UP and DOWN combined",
       subtitle = "The UP with LOG2FOLD > 0 and the DOWN with LOG2FOLD < 0") + 
  my_plot_themes + theme(axis.text.y = element_text(size=12))
ColumnPlot
ggsave(ColumnPlot,
       file = "iModulons_W2.MTb.MetaGeneSets.ALL_NucleicAcid.pdf",
       path = "GSEA_Figures/MetaGeneSets/iModulons/W2vsBroth",
...
# Filter
MetaGeneSets_W2vsBroth_ALL_iModulons_FattyAcid.Cholesterol <- MetaGeneSets_W2vsBroth_ALL_iModulons %>% filter(str_detect(PathName, pattern))

ColumnPlot <- MetaGeneSets_W2vsBroth_ALL_iModulons_FattyAcid.Cholesterol %>% 
  mutate(PathName = str_wrap(PathName, width = 60)) %>%  # Adjust width as needed
  arrange(desc(LOG2FOLD)) %>%
  ggplot(aes(reorder(PathName, LOG2FOLD), LOG2FOLD)) + 
  geom_col(aes(fill = AVG_PVALUE < 0.05)) + 
  ...
  labs(x="FattyAcid/Cholesterol iModulon", y="LOG2FOLD",
       title="FattyAcid/Cholesterol iModulons in W2 Sputum vs Log broth \nusing UP and DOWN combined",
       subtitle = "The UP with LOG2FOLD > 0 and the DOWN with LOG2FOLD < 0") + 
  my_plot_themes + theme(axis.text.y = element_text(size=12))
ColumnPlot
ggsave(ColumnPlot,
       file = "iModulons_W2.MTb.MetaGeneSets.ALL_FattyAcid.Cholesterol.pdf",
       path = "GSEA_Figures/MetaGeneSets/iModulons/W2vsBroth",
       width = 10, height = 6, units = "in")
...
# Filter
MetaGeneSets_W2vsBroth_ALL_iModulons_Metal <- MetaGeneSets_W2vsBroth_ALL_iModulons %>% filter(str_detect(PathName, pattern))

ColumnPlot <- MetaGeneSets_W2vsBroth_ALL_iModulons_Metal %>% 
  mutate(PathName = str_wrap(PathName, width = 60)) %>%  # Adjust width as needed
  arrange(desc(LOG2FOLD)) %>%
  ggplot(aes(reorder(PathName, LOG2FOLD), LOG2FOLD)) + 
  geom_col(aes(fill = AVG_PVALUE < 0.05)) + 
  ...
  labs(x="Metal iModulon", y="LOG2FOLD",
       title="Metal iModulons in W2 Sputum vs Log broth \nusing UP and DOWN combined",
       subtitle = "The UP with LOG2FOLD > 0 and the DOWN with LOG2FOLD < 0") + 
  my_plot_themes + theme(axis.text.y = element_text(size=12))
ColumnPlot
ggsave(ColumnPlot,
       file = "iModulons_W2.MTb.MetaGeneSets.ALL_Metal.pdf",
       path = "GSEA_Figures/MetaGeneSets/iModulons/W2vsBroth",
       width = 10, height = 6, units = "in")
...
# Filter
MetaGeneSets_W2vsBroth_ALL_iModulons_SulfurMetabolism <- MetaGeneSets_W2vsBroth_ALL_iModulons %>% filter(str_detect(PathName, pattern))

ColumnPlot <- MetaGeneSets_W2vsBroth_ALL_iModulons_SulfurMetabolism %>% 
  mutate(PathName = str_wrap(PathName, width = 60)) %>%  # Adjust width as needed
  arrange(desc(LOG2FOLD)) %>%
  ggplot(aes(reorder(PathName, LOG2FOLD), LOG2FOLD)) + 
  geom_col(aes(fill = AVG_PVALUE < 0.05)) + 
  ...
  labs(x="Sulfur Metabolism iModulon", y="LOG2FOLD",
       title="Sulfur Metabolism iModulons in W2 Sputum vs Log broth \nusing UP and DOWN combined",
       subtitle = "The UP with LOG2FOLD > 0 and the DOWN with LOG2FOLD < 0") + 
  my_plot_themes + theme(axis.text.y = element_text(size=12))
ColumnPlot
ggsave(ColumnPlot,
       file = "iModulons_W2.MTb.MetaGeneSets.ALL_SulfurMetabolism.pdf",
       path = "GSEA_Figures/MetaGeneSets/iModulons/W2vsBroth",
       width = 10, height = 3, units = "in")
...
# Filter
MetaGeneSets_W2vsBroth_ALL_iModulons_Growth <- MetaGeneSets_W2vsBroth_ALL_iModulons %>% filter(str_detect(PathName, pattern))

ColumnPlot <- MetaGeneSets_W2vsBroth_ALL_iModulons_Growth %>% 
  mutate(PathName = str_wrap(PathName, width = 60)) %>%  # Adjust width as needed
  arrange(desc(LOG2FOLD)) %>%
  ggplot(aes(reorder(PathName, LOG2FOLD), LOG2FOLD)) + 
  geom_col(aes(fill = AVG_PVALUE < 0.05)) + 
  ...
  labs(x="Growth iModulon", y="LOG2FOLD",
       title="Growth iModulons in W2 Sputum vs Log broth \nusing UP and DOWN combined",
       subtitle = "The UP with LOG2FOLD > 0 and the DOWN with LOG2FOLD < 0") + 
  my_plot_themes + theme(axis.text.y = element_text(size=12))
# ColumnPlot
# ggsave(ColumnPlot,
#        file = "iModulons_W2.MTb.MetaGeneSets.ALL_Growth.pdf",
#        path = "GSEA_Figures/MetaGeneSets/iModulons/W2vsBroth",
#        width = 10, height = 3, units = "in")
...
# Filter
MetaGeneSets_W2vsBroth_ALL_iModulons_Redox <- MetaGeneSets_W2vsBroth_ALL_iModulons %>% filter(str_detect(PathName, pattern))

ColumnPlot <- MetaGeneSets_W2vsBroth_ALL_iModulons_Redox %>% 
  mutate(PathName = str_wrap(PathName, width = 60)) %>%  # Adjust width as needed
  arrange(desc(LOG2FOLD)) %>%
  ggplot(aes(reorder(PathName, LOG2FOLD), LOG2FOLD)) + 
  geom_col(aes(fill = AVG_PVALUE < 0.05)) + 
  scale_fill_manual(values=c("FALSE" = "#999999", "TRUE" = "red3")) + 
  coord_flip() +
  geom_text(aes(y = -1.98, label = paste0("n = ", N_Genes)), hjust = 0, size = 2.5) +
  scale_y_continuous(limits = c(-2, 1), expand = c(0, 0)) +
  labs(x="Redox iModulon", y="LOG2FOLD",
       title="Redox iModulons in W2 Sputum vs Log broth \nusing UP and DOWN combined",
       subtitle = "The UP with LOG2FOLD > 0 and the DOWN with LOG2FOLD < 0") + 
  my_plot_themes + theme(axis.text.y = element_text(size=12))
ColumnPlot
ggsave(ColumnPlot,
       file = "iModulons_W2.MTb.MetaGeneSets.ALL_Redox.pdf",
       path = "GSEA_Figures/MetaGeneSets/iModulons/W2vsBroth",
       width = 10, height = 6, units = "in")
...
# Filter
MetaGeneSets_W2vsBroth_ALL_iModulons_AcidStress <- MetaGeneSets_W2vsBroth_ALL_iModulons %>% filter(str_detect(PathName, pattern))

ColumnPlot <- MetaGeneSets_W2vsBroth_ALL_iModulons_AcidStress %>% 
  mutate(PathName = str_wrap(PathName, width = 60)) %>%  # Adjust width as needed
  arrange(desc(LOG2FOLD)) %>%
  ggplot(aes(reorder(PathName, LOG2FOLD), LOG2FOLD)) + 
  geom_col(aes(fill = AVG_PVALUE < 0.05)) + 
  ...
  labs(x="Acid Stress iModulon", y="LOG2FOLD",
       title="Acid Stress iModulons in W2 Sputum vs Log broth \nusing UP and DOWN combined",
       subtitle = "The UP with LOG2FOLD > 0 and the DOWN with LOG2FOLD < 0") + 
  my_plot_themes + theme(axis.text.y = element_text(size=12))
ColumnPlot
ggsave(ColumnPlot,
       file = "iModulons_W2.MTb.MetaGeneSets.ALL_AcidStress.pdf",
       path = "GSEA_Figures/MetaGeneSets/iModulons/W2vsBroth",
       width = 10, height = 3, units = "in")
...
# Filter
MetaGeneSets_W2vsBroth_ALL_iModulons_Antibiotic <- MetaGeneSets_W2vsBroth_ALL_iModulons %>% filter(str_detect(PathName, pattern))

ColumnPlot <- MetaGeneSets_W2vsBroth_ALL_iModulons_Antibiotic %>% 
  mutate(PathName = str_wrap(PathName, width = 60)) %>%  # Adjust width as needed
  arrange(desc(LOG2FOLD)) %>%
  ggplot(aes(reorder(PathName, LOG2FOLD), LOG2FOLD)) + 
  geom_col(aes(fill = AVG_PVALUE < 0.05)) + 
  ...
  labs(x="Antibiotic iModulon", y="LOG2FOLD",
       title="Antibiotic iModulons in W2 Sputum vs Log broth \nusing UP and DOWN combined",
       subtitle = "The UP with LOG2FOLD > 0 and the DOWN with LOG2FOLD < 0") + 
  my_plot_themes + theme(axis.text.y = element_text(size=12))
ColumnPlot
ggsave(ColumnPlot,
       file = "iModulons_W2.MTb.MetaGeneSets.ALL_Antibiotic.pdf",
       path = "GSEA_Figures/MetaGeneSets/iModulons/W2vsBroth",
       width = 10, height = 6, units = "in")
...
# Filter
MetaGeneSets_W2vsBroth_ALL_iModulons_Virulence.Persistence <- MetaGeneSets_W2vsBroth_ALL_iModulons %>% filter(str_detect(PathName, pattern))

ColumnPlot <- MetaGeneSets_W2vsBroth_ALL_iModulons_Virulence.Persistence %>% 
  mutate(PathName = str_wrap(PathName, width = 60)) %>%  # Adjust width as needed
  arrange(desc(LOG2FOLD)) %>%
  ggplot(aes(reorder(PathName, LOG2FOLD), LOG2FOLD)) + 
  geom_col(aes(fill = AVG_PVALUE < 0.05)) + 
  ...
  labs(x="Virulence/Persistence iModulon", y="LOG2FOLD",
       title="Virulence/Persistence iModulons in W2 Sputum vs Log broth \nusing UP and DOWN combined",
       subtitle = "The UP with LOG2FOLD > 0 and the DOWN with LOG2FOLD < 0") + 
  my_plot_themes + theme(axis.text.y = element_text(size=12))
ColumnPlot
ggsave(ColumnPlot,
       file = "iModulons_W2.MTb.MetaGeneSets.ALL_Virulence.Persistence.pdf",
       path = "GSEA_Figures/MetaGeneSets/iModulons/W2vsBroth",
       width = 10, height = 6, units = "in")
